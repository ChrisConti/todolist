...
const Sommeil = ({ navigation, tasks }) => {
  ...
  const processTasks = () => {
    try {
      //console.log("Processing tasks for sommeil");
      ...
      tasks.forEach((task) => {
        ...
        const taskDuration = parseFloat(task.label);

        if (isNaN(taskDuration)) {
          console.warn(`Invalid task duration for task ID ${task.id}: ${task.label}`);
          return;
        }

        if (taskDate.isSame(moment(), 'day')) {
          todaySum += taskDuration;
        }
        if (taskDate.isSame(moment().subtract(1, 'day'), 'day')) {
          yesterdaySum += taskDuration;
        }
        if (taskDate.isAfter(moment().subtract(7, 'days'))) {
          lastSevenDaysSum += taskDuration;
        }

        for (let i = 0; i < 7; i++) {
          if (taskDate.isSame(moment().subtract(i, 'days'), 'day')) {
            lastSevenDaysData[6 - i] += taskDuration;
          }
        }
        ...
        <View>
          ...
          <View style={{ marginBottom: 20 }}>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-around'  }}>
              ...
              <View style={{  }}>
                <Text>{todaySum} min</Text>
                <Text>{yesterdaySum} min</Text>
                <Text>{lastSevenDaysSum} min </Text>
              </View>
            </View>
          </View>
          <View>
